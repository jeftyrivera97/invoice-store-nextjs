...
export const InvoiceProductAddComponent = () => {

  const { fetchProductoByCodigo, applyDiscount } = useCartStore();

  const [codigoProducto, setCodigoProducto] = useState("");
  const [cantidad, setCantidad] = useState(1);
  const codigoProductoRef = useRef<HTMLInputElement>(null);

  const onAddCartButon = () => {
    if (codigoProducto) {
    ...
    <div className="grid auto-rows-min w-full gap-4 md:grid-cols-1 border border-gray-600 rounded p-4">
      ...
      <div>
        <div className="grid w-full max-w-sm items-center gap-3">
          ...
          <Button size="sm" variant="secondary" onClick={onAddCartButon}>
            <PlusCircleIcon />
            Agregar
          </Button>
        </div>
